...
lazy val `jsoniter-scala-next-tests` = crossProject(JVMPlatform, JSPlatform, NativePlatform)
  ...
  .settings(noPublishSettings)
  .settings(
    crossScalaVersions := Seq("3.8.4-RC2"),
    scalacOptions ++= (CrossVersion.partialVersion(scalaVersion.value) match {
      case Some((2, _)) => Seq()
      case _ => Seq(
        "-Yexplicit-nulls"
...
lazy val `jsoniter-scala-benchmark` = crossProject(JVMPlatform, JSPlatform)
  ...
  .settings(noPublishSettings)
  .settings(
    crossScalaVersions := Seq("3.8.4-RC2", "2.13.18"),
    scalacOptions ++= (CrossVersion.partialVersion(scalaVersion.value) match {
      case Some((2, _)) => Seq(
        "-Wopt",
        "-opt:l:inline",
    ...
    libraryDependencies ++= Seq(
      "com.disneystreaming.smithy4s" %%% "smithy4s-json" % "0.19.4",
      "com.evolutiongaming" %%% "play-json-jsoniter" % "0.10.3" intransitive(),
      "org.playframework" %%% "play-json" % "3.0.6",
      "dev.zio" %%% "zio-blocks-schema" % "0.0.38",
      ...
      "tools.jackson.module" %% "jackson-module-scala" % "3.1.3",
      ...
      case Some((2, _)) => Seq(
        ...
        "com.avsystem.commons" %%% "commons-core" % "2.28.0",
...
lazy val `jsoniter-scala-benchmarkJVM` = `jsoniter-scala-benchmark`.jvm
  ...
  .settings(
    ...
    ThisBuild / assemblyMergeStrategy := {
      case x if x.endsWith("module-info.class") || x.endsWith("LICENSE") => MergeStrategy.discard
