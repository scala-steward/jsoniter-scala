...
object JsonCodecMaker {
  ...
  private[macros] object Impl {
    ...
    def makeOpenapiLike[A: Type](discriminatorFieldName: Expr[String])(using Quotes): Expr[JsonValueCodec[A]] =
      make(CodecMakerConfig.withTransientEmpty(false).withTransientDefault(false).withRequireDiscriminatorFirst(false)
        .withRequireCollectionFields(true).withAllowRecursiveTypes(true)
    ...
    def makeOpenapiLike[A: Type](discriminatorFieldName: Expr[String],
      ...
      make(CodecMakerConfig.withTransientEmpty(false).withTransientDefault(false).withRequireCollectionFields(true)
        .withAllowRecursiveTypes(true).withRequireDiscriminatorFirst(false)
        .withDiscriminatorFieldName(Some(discriminatorFieldName.valueOrAbort))
    ...
    private def make[A: Type](cfg: CodecMakerConfig)(using Quotes): Expr[JsonValueCodec[A]] = {
      ...
      val arrayOfAnyTpe = defn.ArrayClass.typeRef.appliedTo(anyTpe)
      val iArrayOfAnyRefTpe = TypeRepr.of[IArray[AnyRef]]
      val stringTpe = TypeRepr.of[String]
      val tupleTpe = TypeRepr.of[Tuple]
      val newArray = Select(New(TypeIdent(defn.ArrayClass)), defn.ArrayClass.primaryConstructor)
      val newArrayOfAny = TypeApply(newArray, List(Inferred(anyTpe)))
      val fromIArrayMethod = Select.unique(Ref(Symbol.requiredModule("scala.runtime.TupleXXL")), "fromIArray")
      val asInstanceOfMethod = anyTpe.typeSymbol.methodMember("asInstanceOf").head
      ...
      def isTuple(tpe: TypeRepr): Boolean = tpe <:< tupleTpe

      def isGenericTuple(tpe: TypeRepr): Boolean = !defn.isTupleClass(tpe.typeSymbol)
      ...
      def hasSealedParent(tpe: TypeRepr): Boolean = isSealedClass(tpe) || tpe.baseClasses.exists(_.flags.is(Flags.Sealed))
      ...
      def isCollection(tpe: TypeRepr): Boolean = tpe <:< TypeRepr.of[Iterable[?]] ||
      ...
      case class FieldInfo(symbol: Symbol, mappedName: String, getterOrField: Symbol, defaultValue: Option[Term],
                           resolvedTpe: TypeRepr, isTransient: Boolean, isStringified: Boolean, nonTransientFieldIndex: Int)

      abstract class TypeInfo(val paramLists: List[List[FieldInfo]]) {
      ...
      case class ClassInfo(tpe: TypeRepr, tpeTypeArgs: List[TypeRepr], primaryConstructor: Symbol,
                           override val paramLists: List[List[FieldInfo]]) extends TypeInfo(paramLists) {
        def genNew(argss: List[List[Term]]): Term =
      ...
      def genWriteMapScala[K: Type, V: Type](x: Expr[collection.Map[K, V]],
      ...
      def genWriteMapAsArrayScala[K: Type, V: Type](x: Expr[collection.Map[K, V]],
                                                    writeKey: Quotes ?=> (Expr[JsonWriter], Expr[K]) => Expr[Unit],
                                                    writeVal: Quotes ?=> (Expr[JsonWriter], Expr[V]) => Expr[Unit],
                                                    out: Expr[JsonWriter])(using Quotes): Expr[Unit] = '{
        $out.writeArrayStart()
      ...
      val decodeMethodRefs = new mutable.HashMap[DecoderMethodKey, Ref]
      ...
      val encodeMethodRefs = new mutable.HashMap[EncoderMethodKey, Ref]
      ...
      case class ReadDiscriminator(refOpt: Option[Ref]) {
      ...
      def genReadNonAbstractScalaClass[T: Type](typeInfo: TypeInfo, types: List[TypeRepr], useDiscriminator: Boolean,
              ...
              fDefault match {
                ...
                case None =>
                  if (cfg.transientEmpty && fTpe <:< TypeRepr.of[Iterable[?]]) '{
                    ...
                    ${genWriteConstantKey(fieldInfo.mappedName, out)}
                    ${genWriteVal(getter.asInstanceOf[Expr[ft]], fTpe :: types, fieldInfo.isStringified, None, out)}
                  }
              }
          }).asTerm
        }
